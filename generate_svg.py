...
def make_svg(tracks):
    ...
    for i, t in enumerate(tracks):
        ...
        if t["img_b64"]:
            ...
            img_tag = f'''<rect x="16" y="{y}" width="40" height="40" rx="4" fill="{c}"/>
            <text x="36" y="{y+25}" font-family="'Segoe UI',sans-serif" font-size="14" fill="#888" text-anchor="middle">{i+1}</text>'''
        ...
        <g>
          ...
          <text x="490" y="{y+16}" font-family="'Segoe UI',sans-serif" font-size="11" fill="#555" text-anchor="end">{"now playing" if t["np"] else ""}</text>
        </g>
        """
